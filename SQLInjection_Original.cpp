...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Change: normalize SQL and block the specific injection pattern "OR value=value;"
    std::string normalized = sql;
    std::transform(normalized.begin(), normalized.end(), normalized.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });

    const bool hasWhereClause = (normalized.find(" where ") != std::string::npos);

    const std::regex injectionPattern(
        R"(\bor\b\s+('([^']*)'|\d+(\.\d+)?)\s*=\s*('([^']*)'|\d+(\.\d+)?)\s*;?\s*$)",
        std::regex_constants::ECMAScript
    );

    if (hasWhereClause && std::regex_search(normalized, injectionPattern))
    {
        std::cout << "SECURITY ALERT: Suspected SQL injection detected. Query blocked." << std::endl;
